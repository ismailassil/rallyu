...
function serviceUnavailable(reply: any) {
	reply.code(503).send({
		statusCode: 503,
...
const unv_response = {
	replyOptions: {
		onResponse(_request: any, reply: any, res: any) {
			if (reply.statusCode === 500) return serviceUnavailable(reply);
			reply.send(res.stream);
		},
		onError(reply: any, error: any) {
			const code = error?.error?.statusCode || error?.statusCode || 500;

			if (code === 500) return serviceUnavailable(reply);
...
const endpointsPlugin = fp(async (fastify: FastifyInstance, opts: proxiesOpts) => {
	const { AUTH_PORT, NOTIF_PORT, CHAT_PORT, TOURNAMENT_PORT, MATCHMAKING_PORT, GAME_PORT } = opts;
