...
const ConversationBody = ({ user, receiverId }: { user: User, receiverId: number }) => {
  ...
  return (
    <div className="w-11/12 border h-full border-white/30 rounded-lg flex flex-col bg-white/5">
      ...
      <div className='flex gap-4 p-4 pl-6 border-b border-b-white/30'>
        ...
        <div className='flex flex-col justify-center'>
          ...
          <div className='flex items-center gap-2'>
            <span className={`h-3 w-3 rounded-full ${isConnected ? 'bg-green-700' : 'bg-red-700'}`}></span>
            <span className=" font-bold text-gray-400">{isConnected ? 'Online' : 'Offline'}</span>
          </div>
        </div>
        <div className='ml-auto my-auto flex gap-4 h-8'>
          <button className=' rounded-md px-4 bg-white/10 hover:cursor-pointer hover:bg-green-700 transition duration-500'>Play</button>
          <button className=' rounded-md px-4 bg-white/10 hover:cursor-pointer hover:bg-red-700 transition duration-500'>Block</button>
        </div>
      </div>
      {/* message body */}
      ...
      <div className='border-t border-t-white/30 p-4 mt-auto '>
        <div className='flex focus-within:bg-white/12 focus:border-red-400
									duration-200 transition-all bg-white/8 p-3 rounded-lg justify-between focus-within:ring-2 focus-within:ring-white/18'>
          <input
            type='text'
