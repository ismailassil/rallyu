...
const Overlay = ({ status, result, round, tournamentId }: { status: string, result: string | null, round: number, tournamentId?: number | null }) => { // status: gameover wait play countdown none | result: win loss draw
        ...
        <div className={`absolute inset-0 w-full ${status !== 'gameover' && 'pointer-events-none'} h-full`}>
            <div className={`absolute inset-0  rounded-lg transition-all duration-150 w-full h-full`}>
                { tournamentId && result
                    ...
                    : result && <GameOverRemote display={result} game="tictactoe" />}
