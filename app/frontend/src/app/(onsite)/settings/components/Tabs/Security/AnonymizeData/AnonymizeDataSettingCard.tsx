				...
				<div className="p-6 sm:p-8">
					<div className="flex items-center gap-3 mb-8">
						<div className="p-3 bg-blue-500/20 rounded-xl">
							<Shredder className="text-blue-400" size={24} />
						</div>
					...
					<div className="bg-blue-950/30 border border-blue-900/50 rounded-xl p-5">
						<p className="text-gray-300 leading-relaxed">
							{t('text1')} <span className="text-blue-400 font-semibold">{t('text2')}</span> {t('text3')} <br></br>{t('text4')} <code className="text-gray-400">{t('text5')}</code>.
						</p>
...
export default function AnonymizeDataSettingCard() {
	const t = useTranslations('settings.security.cards.anonymize_data');
	...
	return (
		...
		<SettingsCard
			...
			actionButtonOptions={{
				title: t('button'),
				icon: <Shredder size={16} />,
				onClick: () => setIsModalOpen(true)
			}}
		>
		</SettingsCard>
		<AnimatePresence>
			{isModalOpen && <AnonymizeDataModal onCloseModal={() => setIsModalOpen(false)} />}
		</AnimatePresence>
		</>
	);
}
