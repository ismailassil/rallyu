...
export default function FriendsList() {
	...
	const {
		...
		data: friends,
		refetch,
	} = useAPIQuery(() => apiClient.user.fetchFriends());
	...
	// if (!friends || friends.length === 0) return <PlaceholderComponent content={t('no-data')} />;
	...
	return (
		<div>
			{showSkeleton ? (
				...
				<PlaceholderComponent content={t('error')} />
			) : !friends || friends.length === 0 ? (
				<PlaceholderComponent content={t('no-data')} />
			) : (
				<UserList
					users={friends}
					actions={[
